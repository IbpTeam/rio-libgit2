...
int git_index_add_from_buffers(
    git_index *index, git_index_entry *source_entry,
    const void *buffer, size_t len, const void *d_buf, size_t d_len)
{
	git_index_entry *entry = NULL;
	int error = 0;
	git_oid id;
	...
	error = git_blob_create_frombuffer(&rid, INDEX_OWNER(index), d_buf, d_len);
	if (error < 0) {
		index_entry_free(entry);
		return error;
	}
